...
function mixColumns(state) {
  for (var column = 0; column < 4; column++) {
    ...
    state[offset] = gmul(2, s0) ^ gmul(3, s1) ^ s2 ^ s3;
    state[offset + 1] = s0 ^ gmul(2, s1) ^ gmul(3, s2) ^ s3;
    state[offset + 2] = s0 ^ s1 ^ gmul(2, s2) ^ gmul(3, s3);
    state[offset + 3] = gmul(3, s0) ^ s1 ^ s2 ^ gmul(2, s3);
  }
}
...
function invMixColumns(state) {
  for (var column = 0; column < 4; column++) {
    ...
    state[offset] = gmul(0x0e, s0) ^ gmul(0x0b, s1) ^ gmul(0x0d, s2) ^ gmul(0x09, s3);
